<?php
	...
	switch ($_POST['form']) {
		case 'importcsv':
			...
			$yearto = date('Y', strtotime($_POST['dateTo']));
			$monthfrom = date('m', strtotime($_POST['dateFrom']));
			$monthto = date('m', strtotime($_POST['dateTo']));
			$dayfrom = date('d', strtotime($_POST['dateFrom']));
			$dayto = date('d', strtotime($_POST['dateTo']));
			...
			while ( $rowmall = mysqli_fetch_array($resmall) ) {
				...
				while ( $rowcomp = mysqli_fetch_array($rescomp) ) {
					...
					for ( $a = $yearfrom; $a<= $yearto; $a++ ) {

						for ( $b = $monthfrom; $b<= $monthto; $b++ ) {
							$lastdayofmonth = date('t', strtotime($a . "-" . str_pad($b, 2, 0, STR_PAD_LEFT) . "-01"));

							for ( $c = $dayfrom; $c<= $lastdayofmonth; $c++ ) {
								// echo $a."-".str_pad($b, 2, 0, STR_PAD_LEFT)."-".str_pad($c, 2, 0, STR_PAD_LEFT)."|";
						$sales = " LOAD DATA INFILE 'C:/wamp/www/csv/". $rowmall[0] ."/". $rowcomp[0] ."/". $a."-".str_pad($b, 2, 0, STR_PAD_LEFT)."-".str_pad($c, 2, 0, STR_PAD_LEFT) ."/sales.csv' INTO TABLE db_sales
							...
							$discount = " LOAD DATA INFILE 'C:/wamp/www/csv/". $rowmall[0] ."/". $rowcomp[0] ."/". $a."-".str_pad($b, 2, 0, STR_PAD_LEFT)."-".str_pad($c, 2, 0, STR_PAD_LEFT) ."/discount.csv' INTO TABLE db_discount
								FIELDS TERMINATED BY ','
								LINES TERMINATED BY '\r\n' 
								IGNORE 1 LINES 
							...
							$paymenttypes = " LOAD DATA INFILE 'C:/wamp/www/csv/". $rowmall[0] ."/". $rowcomp[0] ."/". $a."-".str_pad($b, 2, 0, STR_PAD_LEFT)."-".str_pad($c, 2, 0, STR_PAD_LEFT) ."/paymenttypes.csv' INTO TABLE db_paymenttypes
								FIELDS TERMINATED BY ','
								LINES TERMINATED BY '\r\n' 
								IGNORE 1 LINES 
							...
							$perhour = " LOAD DATA INFILE 'C:/wamp/www/csv/". $rowmall[0] ."/". $rowcomp[0] ."/". $a."-".str_pad($b, 2, 0, STR_PAD_LEFT)."-".str_pad($c, 2, 0, STR_PAD_LEFT) ."/perhour.csv' INTO TABLE db_perhour
								FIELDS TERMINATED BY ','
								LINES TERMINATED BY '\r\n' 
								IGNORE 1 LINES
							...
							$void = " LOAD DATA INFILE 'C:/wamp/www/csv/". $rowmall[0] ."/". $rowcomp[0] ."/". $a."-".str_pad($b, 2, 0, STR_PAD_LEFT)."-".str_pad($c, 2, 0, STR_PAD_LEFT) ."/void.csv' INTO TABLE db_void
								FIELDS TERMINATED BY ','
								LINES TERMINATED BY '\r\n' 
								IGNORE 1 LINES 
							...
							$sync = " INSERT INTO `db_syncfilestat` SET `tenantID` = '". $rowcomp[0] ."', `sales` = '". $varsales ."', `discount` = '". $vardiscount ."', `void_refund` = '". $varvoid ."', `salesperhour` = '". $varperhour ."', `paymenttype` = '". $varpaymenttypes ."', reportDate = '".$a."-".str_pad($b, 2, 0, STR_PAD_LEFT)."-".str_pad($c, 2, 0, STR_PAD_LEFT)."', countSync = '". $alltotal ."', refno = '". $newid ."' ";
