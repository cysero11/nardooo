...
<?php
	...
	switch($_POST['form']){
		case 'showinquiryreport':
			...
			$sql = " SELECT Inquiry_ID, Company_Name, Trade_Name, UnitType, date_inquired, datefrom, dateto FROM tbltrans_inquiry WHERE (Application_ID = '') AND date_inquired BETWEEN '".date('Y-m-d', strtotime($_POST['dateFrom'])) ."' AND '".date('Y-m-d', strtotime($_POST['dateTo'])) ."' ORDER BY date_inquired DESC LIMIT ".$limit.",20 ";
			$res = mysql_query($sql, $connection);
			while($row = mysql_fetch_array($res)){
				$sql1="SELECT xremarks FROM tbltrans_remarks WHERE inqID= '".$row[0]."'";
				$res1 = mysql_query($sql1, $connection);
				while($row1=mysql_fetch_array($res1)){

					$remarks .= "<span class='fa fa-circle'></span> ".$row1[0]. "<br/>";
				}

				$table .= "
							<tr>
							  	<td width='10%'>".$row[0]."</td>
							  	<td width='10%'>".date("F d,Y",strtotime($row[4]))."</td>
		...
		case 'loadentries2':
	     	...
	     	$limit = ($page-1) * 20;
	     	 ...
	     	 $sql = " SELECT COUNT(*) FROM tbltrans_inquiry WHERE (Application_ID = '') AND date_inquired BETWEEN '". date('Y-m-d', strtotime($_POST['dateFrom'])) ."' AND '". date('	Y-m-d', strtotime($_POST['dateTo'])) ."' ";
	    ...
	    case "loadpageinq_report":
	        $page = $_POST["page"];
	          ...
	          $sqlb = "SELECT COUNT(*) FROM tbltrans_inquiry WHERE (Application_ID = '') AND date_inquired BETWEEN '". date('Y-m-d', strtotime($_POST['dateFrom'])) ."' AND '". date('Y-m-d', strtotime($_POST['dateTo'])) ."' ";
	    ...
	    case 'print_inq_report':
      	$sql = " SELECT Inquiry_ID, Company_Name, Trade_Name, UnitType, date_inquired, datefrom, dateto FROM tbltrans_inquiry WHERE (Application_ID = '') AND date_inquired BETWEEN '".date('Y-m-d', strtotime($_POST['dateFrom'])) ."' AND '".date('Y-m-d', strtotime($_POST['dateTo'])) ."' ORDER BY date_inquired DESC";
        $res = mysql_query($sql, $connection);
        while($row = mysql_fetch_array($res)){
        ...
        $tables .= "
                    <tr>
                      <td>". $row[0] ."</td>
                      <td>". date("F d, Y", strtotime($row[4])) ."</td>
                      <td>". $row[2] ."</td>
                      <td>". $row[3] ."</td>
                      <td>". $row[1] ."</td>
                      <td>". $row[5] ."</td>
