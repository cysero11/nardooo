...
include ("connect.php");
 ...
 $dbhandle = new mysqli($hostdb, $userdb, $passdb, $namedb);
   ...
   $strQuery = "SELECT YEAR(a.`Batch_Date`), SUM(a.`Debit`) AS debit, b.`HeaderDesc` FROM gltransjournal AS a
LEFT JOIN refchart AS b ON a.`GLAccount_Code` = b.`GLAccount_Code`
WHERE b.`SubheaderDesc` = 'General, Selling and Administrative Expenses'
...
GROUP BY b.`Category` order by SUM(a.QtyActual * a.Cost) DESC*/
	...
	// Push the data into the array
        	...
        	$columnChart = new FusionCharts("column2d", "exp2" , "100%", 320, "e2", "json", $jsonEncodedData);
