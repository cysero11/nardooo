...
include ("connect_acctg.php");
 ...
 $dbhandle = new mysqli($hostdb, $userdb, $passdb, $namedb);
   ...
   $strQuery = "SELECT YEAR(a.`Batch_Date`), SUM(CASE WHEN b.`SubheaderDesc` = 'Prepayments and other current assets' THEN b.`BeginningBal` + (a.`Debit` - a.`Credit`) ELSE 0 END) AS debit, b.`HeaderDesc` FROM gltransjournal AS a
...
GROUP BY b.`Category` order by SUM(a.QtyActual * a.Cost) DESC*/
	...
	// Push the data into the array
        	...
        	$columnChart = new FusionCharts("column2d", "ret3" , "100%", 350, "asset3", "json", $jsonEncodedData);
