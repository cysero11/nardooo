...
$countryCode = $_GET["Month"];
     	...
     	$cityQuery = "select DAY(a.`Batch_Date`), SUM(CASE WHEN b.`SubheaderDesc` = 'Inventories' THEN b.`BeginningBal` + (a.`Debit` - a.`Credit`) ELSE 0 END) AS debit, b.`HeaderDesc` FROM gltransjournal AS a
LEFT JOIN refchart AS b ON a.`GLAccount_Code` = b.`GLAccount_Code`
WHERE b.`SubheaderDesc` = 'Inventories' and MONTHNAME(a.`Batch_Date`) = ?
     	...
     	if ($cityResult) {
        	...
        	$arrData = array(
                "chart" => array(
        			...
        			"valueFontBold" => "1",
					"rotateValues" => "1",
                    "caption" => "Daily Inventories of  ".$countryName,
                    "showValues"=> "1",
                    "theme"=> "zune"
              	)
           	);
