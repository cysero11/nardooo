...
$countryCode = $_GET["Year"];
		...
		SUM(CASE WHEN b.`SubheaderDesc` = 'Intangible Assets' THEN b.`BeginningBal` + (a.`Debit` - a.`Credit`) ELSE 0 END) FROM gltransjournal AS a
     	...
     	if ($cityResult) {
        	...
        	$arrData = array(
                "chart" => array(
        			...
        			"valueFontBold" => "1",
					"rotateValues" => "1",
                    "caption" => "Monthly Intangible Assets of  ".$countryName,
                    "showValues"=> "1",
                    "theme"=> "zune"
              	)
           	);
	...
	// Push the data into the array
        	while($row = $cityResult->fetch_array()) {
                array_push($arrData["data"], array(
              	"label" => $row[0],
              	"value" => $row[1],
				"link" => "intangible2.php?Month=".$row[0],
				"color" => "#F9690E"
              	)
           	);
        	}
