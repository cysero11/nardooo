	...
	include ("fusioncharts.php");
     // Get the country code from the GET parameter
     	...
     	$cityQuery = "select DAY(a.`Batch_Date`), SUM(CASE WHEN b.`SubheaderDesc` = 'Cash and Cash Equivalents' THEN b.`BeginningBal` + (a.`Debit` - a.`Credit`) ELSE 0 END) AS debit, b.`HeaderDesc` FROM gltransjournal AS a
LEFT JOIN refchart AS b ON a.`GLAccount_Code` = b.`GLAccount_Code`
where b.`SubheaderDesc` = 'Cash and Cash Equivalents' and MONTHNAME(a.`Batch_Date`) = ?
     	...
     	if ($cityResult) {
        	...
        	$arrData = array(
                "chart" => array(
        			...
        			"valueFontBold" => "1",
					"rotateValues" => "1",
                    "caption" => "Daily Cash and Cash Equivalents of  ".$countryName,
                    "showValues"=> "1",
                    "theme"=> "zune"
              	)
           	);
