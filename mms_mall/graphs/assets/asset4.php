...
include ("connect_acctg.php");
 ...
 $dbhandle = new mysqli($hostdb, $userdb, $passdb, $namedb);
   ...
   $strQuery = "SELECT YEAR(a.`Batch_Date`), SUM(CASE WHEN b.`SubheaderDesc` = 'Intangible Assets' THEN b.`BeginningBal` + (a.`Debit` - a.`Credit`) ELSE 0 END) AS debit, b.`HeaderDesc` FROM gltransjournal AS a
	...
	// Push the data into the array
        	...
        	while($row = mysqli_fetch_array($result)) {
           	array_push($arrData["data"], array(
                ...
                "value" => $row[1],
				"color" => "#446CB3",
				"link" => "intangible.php?Year=".$row[0]
              	)
           	);
        	} 
        	...
        	$columnChart = new FusionCharts("column2d", "ret4" , "100%", 350, "asset4", "json", $jsonEncodedData);
