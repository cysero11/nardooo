...
$countryCode = $_GET["Year"];
     	...
     	$cityQuery = "select MONTHNAME(a.`Batch_Date`), SUM(CASE WHEN b.`SubheaderDesc` = 'Inventories' THEN b.`BeginningBal` + (a.`Debit` - a.`Credit`) ELSE 0 END) AS debit, b.`HeaderDesc` FROM gltransjournal AS a
     	...
     	if ($cityResult) {
        	...
        	$arrData = array(
                "chart" => array(
        			...
        			"valueFontBold" => "1",
					"rotateValues" => "1",
                    "caption" => "Monthly Inventories of  ".$countryName,
                    "showValues"=> "1",
                    "theme"=> "zune"
              	)
           	);
