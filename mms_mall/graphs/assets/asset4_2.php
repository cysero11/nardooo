...
$countryCode = $_GET["Month"];
     	...
     	$cityQuery = "select DAY(a.`Batch_Date`), SUM(CASE WHEN b.`SubheaderDesc` = 'Intangible Assets' THEN b.`BeginningBal` + (a.`Debit` - a.`Credit`) ELSE 0 END) AS debit, b.`HeaderDesc` FROM gltransjournal AS a
     	...
     	if ($cityResult) {
        	...
        	$arrData = array(
                "chart" => array(
				    "showlegend" => '1',
					"numberPrefix" => "₱",
                    "caption" => "Daily Intangible Assets of  ".$countryName,
                    "showValues"=> "1",
                    "theme"=> "zune"
              	)
           	);
