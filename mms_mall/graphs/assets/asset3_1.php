...
$countryCode = $_GET["Year"];
     	...
     	$cityQuery = "select MONTHNAME(a.`Batch_Date`),  SUM(CASE WHEN b.`SubheaderDesc` = 'Prepayments and other current assets' THEN b.`BeginningBal` + (a.`Debit` - a.`Credit`) ELSE 0 END) AS debit, b.`HeaderDesc` FROM gltransjournal AS a
LEFT JOIN refchart AS b ON a.`GLAccount_Code` = b.`GLAccount_Code`
WHERE YEAR(a.`Batch_Date`) = ?
...
$cityPrepStmt = $dbhandle->prepare($cityQuery);
     	...
     	if ($cityResult) {
        	...
        	$arrData = array(
                "chart" => array(
        			...
        			"valueFontBold" => "1",
					"rotateValues" => "1",
                    "caption" => "Monthly Prepayments and other current assets of  ".$countryName,
                    "showValues"=> "1",
                    "theme"=> "zune"
              	)
           	);
	...
	// Push the data into the array
           ...
           /*JSON Encode the data to retrieve the string containing the JSON representation of the data in the array. */
        	...
        	$columnChart = new FusionCharts("column2d", "oca" , "100%", 450, "quarter", "json", $jsonEncodedData);
