    ...
    switch ($_POST['form']) {
        ...
        case 'tblselectshortcutunit':
            ...
            while ($row = mysql_fetch_array($res)) {
                echo "
                    <tr onclick='slctthisunit(\"".$row[8]."\", \"".$row[9]."\", \"".$row[10]."\", \"".$row[11]."\", \"".$row[12]."\", \"".$row[0]."\", \"".$_POST['txtinq_mallbranch']."\");'>
        ...
        case 'loadinquiry_department':
            ...
            $unitinfo = mysql_fetch_array(mysql_query("SELECT sqmunitsetup, pricepersqmunitsetup, totalamountunitsetup, sqm_width, sqm_height, assocdues, typeofbusiness FROM tblref_unit WHERE unitid = '".$_POST['unit']."'"));
            ...
            $totalmonthlydues = $assocdues + $unitinfo[2];

            echo $dep."|".$cat."|".$wing."|".$floor."|".$unit."|".$unitinfo[0]."|".number_format($unitinfo[1], '2', '.', ',')."|".number_format($unitinfo[2], '2', '.', ',')
            ."|".$unitinfo[3]."|".$unitinfo[4]."|".$lca . "|" . number_format($assocdues, '2', '.', ',') . "|" . number_format($totalmonthlydues, '2', '.', ',') . "|" . $unitinfo[6];
		break;
    }
?>
